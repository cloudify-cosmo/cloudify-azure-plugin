...
@operation
def create_disk(**_):
        disks = ctx.node.properties['no_of_disks']
        curr_disk_key = []
        for x in range(0, disks):
                curr_disk_key.append("{0}{1}{2}{3}".format(constants.DATA_DISK_KEY, ctx.node.id, ctx.instance.id, str(x)))
