...
def create_subnet(**_):
    ...
    ctx.logger.info("In create_subnet: {0} is {1}".format(curr_subnet_key, current_subnet_name))
...
def set_subnets_from_runtime(caller_string, source_runtime_properties, target_runtime_properties, use_only_first_subnet=True):
    ...
    for curr_key in target_runtime_properties:
        if curr_key.startswith(constants.SUBNET_KEY):
            ...
            ctx.logger.info("set_subnets_from_runtime: {0}:{1} is {2}".format(caller_string, curr_key, source_runtime_properties[curr_key]))
            if key_index == 0:
