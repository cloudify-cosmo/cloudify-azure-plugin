...
def clear_runtime_properties(delete_instance_file=True):
    if constants.RESOURCE_NOT_DELETED in ctx.instance.runtime_properties:
        if ctx.instance.runtime_properties[constants.RESOURCE_NOT_DELETED]:
            return
    ...
    try:
        ...
        with open(current_runtime_file_path, 'a') as f:
            for curr_runtime_property in ctx.target.instance.runtime_properties:
                ...
                ctx.logger.info("orig_runtime_property is {0}".format(orig_runtime_property))
...
def get_value_from_node(current_key, store_in_runtime):
    current_node = get_node_or_source_node()
    if 'azure_config' in current_node.properties:
        current_azure_config = current_node.properties['azure_config']
