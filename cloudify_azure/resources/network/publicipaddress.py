...
def get_unique_name(public_ip_address, resource_group_name, name):
    if not name:
        for _ in range(0, 15):
            name = "{0}".format(uuid4())
            try:
...
def create(ctx, **_):
    ...
    public_ip_address = PublicIPAddress(azure_config, ctx.logger)
    # generate name if not provided
    name = get_unique_name(public_ip_address, resource_group_name, name)
    ...
    try:
        ...
        if ctx.node.properties.get('use_external_resource', False):
            raise cfy_exc.NonRecoverableError(
                "Can't use non-existing public_ip_address '{0}'.".format(name))
        else:
            try:
                result = \
